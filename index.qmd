...
db = {
  ...
  await configuredClient.query(
    ...
    SELECT CANTVILLE, AGED, SEXE, FROM read_parquet('https://static.data.gouv.fr/resources/recensement-de-la-population-fichiers-detail-individus-localises-au-canton-ou-ville-2020-1/20231023-122841/fd-indcvi-2020.parquet')`)
...
duckdb.sql(f'''
  ...
  AS SELECT CANTVILLE, AGED, SEXE, FROM read_parquet('https://static.data.gouv.fr/resources/recensement-de-la-population-fichiers-detail-individus-localises-au-canton-ou-ville-2020-1/20231023-122841/fd-indcvi-2020.parquet') LIMIT 5
''')
)
```
...
```{ojs}
Inputs.table(
  db.query(`SELECT * FROM franceRP LIMIT 5`)
)
```
...
```{ojs}
Plot.rectY(df_full, Plot.binX({y: "sum"}, {x: "AGED", y: (d) => d.SEXE == 1 ? 1 : -1, fill: "SEXE", tip: true})).plot({y: {label: "Frequency"}})
```
